...
export default function DiscoverQuests() {
  ...
  const { data: myQuests } = useMyQuests(); // defensive check in case cache races
  const join = useJoinQuest();
  const [joiningId, setJoiningId] = React.useState<string | null>(null); // keep as string
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.map((q) => {
            ...
            const onJoin = async () => {
              if (joined) return;
              setJoiningId(qid);
              try { await join.mutateAsync(qid); } // pass string
