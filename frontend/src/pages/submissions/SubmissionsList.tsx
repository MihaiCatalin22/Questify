...
const TABS: Array<SubmissionStatus | "ALL"> = ["PENDING", "APPROVED", "REJECTED", "ALL"];

export default function SubmissionsList() {
  const [tab, setTab] = useState<SubmissionStatus | "ALL">("PENDING");
  const [q, setQ] = useState('');

  const { data, isLoading, isError, error } = useSubmissions(tab);
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {TABS.map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`rounded-full px-3 py-1.5 text-sm border ${tab === t ? 'bg-black text-white' : 'bg-white'}`}
      ...
      {isLoading ? (
        ...
        <div className="text-red-600">{(error as any)?.message || 'Failed to load submissions'}</div>
      ) : (
